...
class FillServiceMeta extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:fill-service-meta';
    ...
    {
        $metas = array();
        $json = file_get_contents(resource_path('jsons/services_01.json'));
            ...
            foreach ($metas as $key => $val) {
                foreach ($val as $k => $value) {
                    if (empty($value['parameter_link']) && empty($value['parameter_category'])) {
                        $meta = ServiceMeta::where('language', $value['language'])->whereNull('parameter_link')->whereNull('parameter_category')->first();
                    } elseif (!empty($value['parameter_link']) && empty($value['parameter_category'])) {
                        $meta = ServiceMeta::where([
