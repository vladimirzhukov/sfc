...
class FillStartupsMeta extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:fill-startups-meta';
    ...
    {
        $metas = array();
        $json = file_get_contents(resource_path('jsons/startups_01.json'));
        $data = json_decode($json, true);
        foreach ($data as $key => $dat) {
            $metas[$key] = $dat;
        }
        $json = file_get_contents(resource_path('jsons/startups_02.json'));
        $data = json_decode($json, true);
        foreach ($data as $key => $dat) {
            $metas[$key] = $dat;
        }
        $json = file_get_contents(resource_path('jsons/startups_03.json'));
        $data = json_decode($json, true);
        foreach ($data as $key => $dat) {
            $metas[$key] = $dat;
        }
            ...
            foreach ($metas as $key => $val) {
                foreach ($val as $k => $value) {
                    if (empty($value['parameter_link']) && empty($value['parameter_category'])) {
                        $meta = StartupMeta::where('language', $value['language'])->whereNull('parameter_link')->whereNull('parameter_category')->first();
                    } elseif (!empty($value['parameter_link']) && empty($value['parameter_category'])) {
                        $meta = StartupMeta::where([
                    ...
                    $meta->save();
                    $this->info('Startup meta for ' . $value['language'] . ' - ' . $value['parameter_link'] . ' - ' . $value['parameter_category'] . ' saved');
                }
            }
        }
    }
