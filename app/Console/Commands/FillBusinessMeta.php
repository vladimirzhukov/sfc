...
class FillBusinessMeta extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:fill-business-meta';
    ...
    {
        $metas = array();
        $json = file_get_contents(resource_path('jsons/businesses_01.json'));
        $data = json_decode($json, true);
        foreach ($data as $key => $dat) {
            $metas[$key] = $dat;
        }
        $json = file_get_contents(resource_path('jsons/businesses_02.json'));
        $data = json_decode($json, true);
        foreach ($data as $key => $dat) {
            $metas[$key] = $dat;
        }
        $json = file_get_contents(resource_path('jsons/businesses_03.json'));
        $data = json_decode($json, true);
        foreach ($data as $key => $dat) {
            $metas[$key] = $dat;
        }
            ...
            foreach ($metas as $key => $val) {
                foreach ($val as $k => $value) {
                    if (empty($value['parameter_link']) && empty($value['parameter_category'])) {
                        $meta = BusinessMeta::where('language', $value['language'])->whereNull('parameter_link')->whereNull('parameter_category')->first();
