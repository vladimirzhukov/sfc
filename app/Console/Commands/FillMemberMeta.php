...
class FillMemberMeta extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:fill-member-meta';
    ...
    {
        $metas = array();
        $json = file_get_contents(resource_path('jsons/members_01.json'));
        $data = json_decode($json, true);
        foreach ($data as $key => $dat) {
            $metas[$key] = $dat;
        }
        $json = file_get_contents(resource_path('jsons/members_02.json'));
        $data = json_decode($json, true);
        foreach ($data as $key => $dat) {
            $metas[$key] = $dat;
        }
            ...
            foreach ($metas as $key => $val) {
                foreach ($val as $k => $value) {
                    if (empty($value['parameter_link']) && empty($value['parameter_category'])) {
                        $meta = MemberMeta::where('language', $value['language'])->whereNull('parameter_link')->whereNull('parameter_category')->first();
                    } elseif (!empty($value['parameter_link']) && empty($value['parameter_category'])) {
                        $meta = MemberMeta::where([
                    ...
                    $meta->page_subnote = $value['page_subnote'] ?? null;
                    $meta->page_seo = $value['page_seo'] ?? null;
                    $meta->save();
