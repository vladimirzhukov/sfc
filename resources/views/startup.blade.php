...
<script type="application/ld+json">
{
    ...
    "url": "{{ $startup->link ?? route('web::startup', $startup->slug) }}",
    @if ($startup->img)
        "logo": "https://fvn.ams3.cdn.digitaloceanspaces.com/sfccy/startups/{{ substr($startup->img, 0, 1) }}/{{ substr($startup->img, 0, 2) }}/{{ substr($startup->img, 0, 3) }}/{{ $startup->img }}",
    @endif
    @if ($startup->founding_year)
        "foundingDate": "{{ $startup->founding_year }}-01-01",
    @endif
    @if (!$startup->is_online && $startup->city)
        "address": {
            "@@type": "PostalAddress",
            "addressLocality": "{{ $startup->city->name }}",
            "addressCountry": "CY"
        },
    @endif
    @if ($startup->lat && $startup->lon)
        "geo": {
            "@@type": "GeoCoordinates",
            "latitude": {{ $startup->lat }},
            "longitude": {{ $startup->lon }}
        },
    @endif
    "sameAs": [
        "{{ route('web::startup', $startup->slug) }}"
        @if ($startup->link)
            ,"{{ $startup->link }}"
        @endif
    ]
        ...
        <div class="mx-auto max-w-4xl">
                ...
                <ol role="list" class="flex items-center space-x-4">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <a href="{{ route('web::startups') }}" class="ml-4 text-sm font-medium text-gray-400 hover:text-gray-300">{{ __('Startups') }}</a>
                        </div>
                    </li>
                    <li>
                        <div class="flex items-center">
                            <svg viewBox="0 0 20 20" fill="currentColor" class="size-5 shrink-0 text-gray-400"><path d="M8.22 5.22a.75.75 0 0 1 1.06 0l4.25 4.25a.75.75 0 0 1 0 1.06l-4.25 4.25a.75.75 0 0 1-1.06-1.06L11.94 10 8.22 6.28a.75.75 0 0 1 0-1.06Z" clip-rule="evenodd" fill-rule="evenodd" /></svg>
                            <span class="ml-4 text-sm font-medium text-gray-500" aria-current="page">{{ Str::limit($startup->name, 30) }}</span>
                        </div>
                    </li>
                </ol>
            </nav>
            ...
            <div class="mb-6">
                @if ($startup->is_fundraising)
                    <span class="inline-flex items-center gap-x-1.5 rounded-full bg-yellow-100 px-3 py-1 text-sm font-medium text-yellow-800">
                    <div class="flex-none rounded-full bg-yellow-500 p-1"><div class="size-1.5 rounded-full bg-white"></div></div>
                    {{ __('Actively Fundraising') }}
                </span>
                @else
            ...
            <h1 class="text-4xl font-bold tracking-tight text-white sm:text-5xl lg:text-6xl">{{ $startup->name }}</h1>
            <div class="mt-8 space-y-4">
                @if ($startup->founding_year)
                    <div class="flex items-center text-lg text-gray-300">
                        ...
                        {{ __('Founded in') }} {{ $startup->founding_year }}
                    </div>
                @endif
                <div class="flex items-center text-lg text-gray-300">
                    ...
                    @if ($startup->is_online)
                        {{ __('Online Startup') }}
                    @else
                        {{ $startup->location ?? ($startup->city->name ?? __('Cyprus')) }}
                    @endif
                </div>
                @if ($startup->is_fundraising)
                    <div class="flex items-center text-lg text-gray-300">
                        <svg class="size-6 mr-3 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1" /></svg>
                        <span class="text-yellow-400 font-semibold">{{ __('Seeking Investment') }}</span>
                    </div>
                @endif
            </div>
            <div class="mt-10 flex flex-wrap gap-4">
                @if ($startup->link)
                    <a href="{{ $startup->link }}" target="_blank" rel="noopener noreferrer" class="rounded-md bg-orange-500 px-6 py-3 text-base font-semibold text-white shadow-sm hover:bg-orange-400 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-orange-500">
                        {{ __('Visit Website') }}
            ...
            <div class="lg:grid lg:grid-cols-12 lg:gap-12">
                <div class="lg:col-span-8">
                    <div class="prose prose-lg max-w-none">
                        <h2 class="text-2xl font-bold text-gray-900 mb-6">{{ __('About this Startup') }}</h2>
                        @if ($startup->description_html)
                            {!! $startup->description_html !!}
                        @elseif ($startup->description)
                            <p class="text-gray-700 leading-relaxed">{!! nl2br(e($startup->description)) !!}</p>
                        @else
                            <p class="text-gray-500 italic">{{ __('No description available for this startup.') }}</p>
                        @endif
                    </div>

                    <h3 class="text-lg font-semibold text-gray-900 mt-8 mb-4">{{ __('Startup Details') }}</h3>
                    <dl class="space-y-4">
                        @if ($startup->founding_year)
                            <div>
                                <dt class="text-sm font-medium text-gray-500">{{ __('Founded') }}</dt>
                                <dd class="mt-1 text-sm text-gray-900">{{ $startup->founding_year }}</dd>
                            </div>
                        @endif
                        <div>
                        ...
                        <div>
                            <dt class="text-sm font-medium text-gray-500">{{ __('Fundraising Status') }}</dt>
                            <dd class="mt-1 text-sm text-gray-900">
                                @if ($startup->is_fundraising)
                                    <span class="text-yellow-600 font-semibold">{{ __('Actively Fundraising') }}</span>
                                @else
                                    <span class="text-green-600 font-semibold">{{ __('Not actively fundraising') }}</span>
                                @endif
                            </dd>
                        </div>
                        @if (!empty($startup->city))
                            <div>
                                <dt class="text-sm font-medium text-gray-500">{{ __('City') }}</dt>
                                <dd class="mt-1 text-sm text-gray-900">{{ $startup->city->name }}</dd>
                            </div>
                        @endif
                    </dl>
                    ...
                    <div class="sticky top-8 space-y-6">
                        @if ($startup->img)
                            <div class="bg-gray-50 rounded-lg p-6">
                                <img src="https://fvn.ams3.cdn.digitaloceanspaces.com/sfccy/startups/{{ substr($startup->img, 0, 1) }}/{{ substr($startup->img, 0, 2) }}/{{ substr($startup->img, 0, 3) }}/{{ $startup->img }}" alt="{{ $startup->name }}" class="w-full object-contain rounded-lg shadow-lg max-h-48">
                            </div>
                        @endif
                        @if (!$startup->is_online && $startup->lat && $startup->lon)
                            <div class="bg-gray-50 rounded-lg p-6">
                                ...
                                <p class="mt-2 text-sm text-gray-600">{{ $startup->location }}</p>
                            </div>
                        @endif
                        @if (!empty($startup->user->profile->id))
                            <div class="bg-gray-50 rounded-lg p-6">
                                <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ __('Founder') }}</h3>
                                <div class="flex items-center space-x-3">
                                    @if ($startup->user->img)
                                        <img src="{{ (!empty($startup->user->img) ? 'https://fvn.ams3.cdn.digitaloceanspaces.com/sfccy/avatars/' . substr($startup->user->img, 0, 1) . '/' . substr($startup->user->img, 0, 2) . '/' . substr($startup->user->img, 0, 3) . '/th_' . $startup->user->img : '/assets/images/no-user-image-icon.png') }}" alt="{{ trim($startup->user->profile->first_name . ' '. $startup->user->profile->last_name) }}" class="size-10 rounded-full">
                                    @else
                                    ...
                                    @endif
                                    <div>
                                        <p class="font-medium text-gray-900">{{ trim($startup->user->profile->first_name . ' '. $startup->user->profile->last_name) }}</p>
                                    </div>
                                </div>
                            </div>
                        @endif
                        <div class="bg-gray-50 rounded-lg p-6">
                            <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ __('Share this Startup') }}</h3>
...
@if (!empty($relatedStartups) && $relatedStartups->isNotEmpty())
    <div class="bg-gray-100 py-16 sm:py-24 dark:bg-gray-900">
        <div class="mx-auto max-w-7xl px-6 lg:px-8">
            <div class="mx-auto max-w-2xl lg:text-center">
                <h2 class="text-2xl font-bold text-gray-900 mb-6">{{ __('Related Startups') }}</h2>
            </div>
            <div class="bg-white shadow overflow-hidden sm:rounded-md dark:bg-gray-800">
                <ul role="list" class="divide-y divide-gray-200 dark:divide-gray-700">
                    @foreach ($relatedStartups as $relatedStartup)
                        <li class="relative flex justify-between gap-x-6 px-4 py-5 hover:bg-gray-50 sm:px-6 lg:px-8 dark:hover:bg-white/2.5">
                            <div class="flex min-w-0 gap-x-4">
                                <img src="{{ (!empty($relatedStartup->img) ? 'https://fvn.ams3.cdn.digitaloceanspaces.com/sfccy/startups/' . substr($relatedStartup->img, 0, 1) . '/' . substr($relatedStartup->img, 0, 2) . '/' . substr($relatedStartup->img, 0, 3) . '/th_' . $relatedStartup->img : '/assets/images/no-image-icon.png') }}" alt="{{ $relatedStartup->name }}" class="size-12 flex-none rounded-lg bg-gray-50 object-cover dark:bg-gray-800" />
                                <div class="min-w-0 flex-auto">
                                    <p class="text-sm/6 font-semibold text-gray-900 dark:text-white">
                                        <a href="{{ route('web::startup', $relatedStartup->slug) }}">
                                    ...
                                    <p class="mt-1 text-xs/5 text-gray-500 dark:text-orange-400">
                                        @php
                                        $categories = '';
                                        if (!empty($relatedStartup->categories)) {
                                            $categoryIDs = explode('][', trim($relatedStartup->categories, '[]'));
                                        ...
                                        @endphp
                                        {{ (!empty($categories) ? $categories . ' • ' : '') }}{{ (!empty($relatedStartup->is_online) ? __('Online') : (!empty($relatedStartup->city->name) ? $relatedStartup->city->name . (!empty($relatedStartup->city->region->name) ? ', ' . $relatedStartup->city->region->name : '') : __('Cyprus'))) }}
                                    </p>
                                    @if (!empty($relatedStartup->description))
                                        <p class="mt-2 text-sm/6 text-gray-600 dark:text-gray-300">{{ Str::limit($relatedStartup->description, 120) }}</p>
                                    @endif
                                    @if (!empty($relatedStartup->location))
                                        <div class="flex items-center gap-x-1.5">
                                            ...
                                            <p class="text-xs/5 text-gray-500 dark:text-gray-400">{{ Str::limit($relatedStartup->location, 40) }}</p>
                                        </div>
                                    @endif
                                </div>
                            </div>
                            <div class="flex shrink-0 items-center gap-x-4">
                                <div class="hidden sm:flex sm:flex-col sm:items-end">
                                    @if (!empty($relatedStartup->is_fundraising))
                                        <div class="flex items-center gap-x-1.5">
                                            ...
                                            <p class="text-xs/5 text-yellow-600 dark:text-yellow-400 font-medium">{{ __('Fundraising') }}</p>
                                        </div>
                                    @else
                                        <div class="flex items-center gap-x-1.5">
                                    ...
                                    @endif
                                    @if (!empty($relatedStartup->founding_year))
                                        <p class="mt-1 text-xs/5 text-gray-500 dark:text-gray-400">{{ __('Founded') }} {{ $relatedStartup->founding_year }}</p>
                                    @endif
                                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof L !== 'undefined') {
        const map = L.map('map', {
            center: [{{ $startup->lat }}, {{ $startup->lon }}],
            zoom: 15,
        ...
        L.marker([{{ $startup->lat }}, {{ $startup->lon }}])
            .addTo(map)
            .bindPopup('{{ addslashes($startup->location ?? $startup->name) }}');
    }
    @endif
    ...
    if (shareBtn) {
        shareBtn.addEventListener('click', function() {
            const shareData = {
                title: {!! json_encode($startup->name) !!},
                text: {!! json_encode(Str::limit(strip_tags($startup->description ?? ''), 100)) !!},
    ...
    modal.innerHTML = `
        <div class="bg-white rounded-lg p-6 max-w-sm mx-4 w-full">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-semibold text-gray-900">{{ __('Share this Startup') }}</h3>
...
function setupSocialShareLinks() {
    const startupTitle = {!! json_encode($startup->name) !!};
    const startupDescription = {!! json_encode(Str::limit(strip_tags($startup->description ?? ''), 100)) !!};
    const startupUrl = window.location.href;
    const startupYear = '{{ $startup->founding_year ?? '' }}';

    // Facebook share
    const facebookBtn = document.getElementById('facebook-share');
    if (facebookBtn) {
        facebookBtn.href = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(startupUrl)}`;
    }

    // Twitter share
    const twitterBtn = document.getElementById('twitter-share');
    if (twitterBtn) {
        const tweetText = startupYear ? `${startupTitle} - Founded ${startupYear}` : startupTitle;
        twitterBtn.href = `https://twitter.com/intent/tweet?text=${encodeURIComponent(tweetText)}&url=${encodeURIComponent(startupUrl)}`;
    }

    // WhatsApp share
    const whatsappBtn = document.getElementById('whatsapp-share');
    if (whatsappBtn) {
        const whatsappText = `Check out this startup: ${startupTitle}${startupYear ? ` (Founded ${startupYear})` : ''} ${startupUrl}`;
    ...
    if (linkedinBtn) {
        linkedinBtn.href = `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(startupUrl)}`;
    }
