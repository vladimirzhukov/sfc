...
<div class="space-y-12">
    @if (!empty($services) && $services->isNotEmpty())
        <div class="sm:flex sm:items-center">
            <div class="sm:flex-auto">
                <h1 class="text-base font-semibold text-gray-900">{{ __('Services') }}</h1>
                <p class="mt-2 text-sm text-gray-700">{{ __('app.service_listing') }}</p>
            </div>
            <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
                <a href="{{ route('app::services::create') }}" class="block rounded-md bg-indigo-600 px-3 py-2 text-center text-sm font-semibold text-white shadow-xs hover:bg-indigo-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600 cursor-pointer">{{ __('Add Service') }}</a>
            </div>
        </div>
            ...
            <div class="-mx-4 -my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
                <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                    <table class="min-w-full divide-y divide-gray-300">
                        <thead>
                        <tr>
                            <th scope="col" class="py-3.5 pr-3 pl-4 text-left text-sm font-semibold text-gray-900 sm:pl-0">{{ __('Service') }}</th>
                                ...
                                <td class="py-5 pr-3 pl-4 text-sm sm:pl-0 w-1/3">
                                    <div class="flex items-center">
                                        <div class="size-11 shrink-0">
                                            <img src="{{ (!empty($service->img) ? 'https://fvn.ams3.cdn.digitaloceanspaces.com/sfccy/services/' . substr($service->img, 0, 1) . '/' . substr($service->img, 0, 2) . '/' . substr($service->img, 0, 3) . '/th_' . $service->img : '/assets/images/no-image-icon.png') }}" alt="{{ $service->name }}" class="size-11 rounded-full" />
                                        </div>
                                        <div class="ml-4 rtl:mr-4 rtl:ml-0">
                                            <div class="font-medium text-gray-900">{{ $service->name }}</div>
                                            <div class="mt-1 text-gray-500">{{ (!empty($service->description_html) ? mb_substr(strip_tags($service->description_html), 0, 100) : '') }}...</div>
                                        </div>
                                    </div>
                                </td>
    ...
    @else
        <div class="text-center">
            ...
            <p class="mt-1 text-sm text-gray-500">{{ __('Get started by adding your service.') }}</p>
            <div class="mt-6"><a href="{{ route('app::services::create') }}" class="inline-flex items-center rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-xs hover:bg-indigo-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600"><svg viewBox="0 0 20 20" fill="currentColor" data-slot="icon" aria-hidden="true" class="mr-1.5 -ml-0.5 size-5"><path d="M10.75 4.75a.75.75 0 0 0-1.5 0v4.5h-4.5a.75.75 0 0 0 0 1.5h4.5v4.5a.75.75 0 0 0 1.5 0v-4.5h4.5a.75.75 0 0 0 0-1.5h-4.5v-4.5Z" /></svg> {{ __('Add Service') }}</a></div>
        </div>
    @endif
</div>
@endsection
