...
<script type="application/ld+json">
{
    ...
    "provider": {
    "@@type": "Person",
    "name": "{{ $service->user->profile->first_name ?? '' }} {{ $service->user->profile->last_name ?? '' }}"
    },
    @if ($service->img)
        "image": "https://fvn.ams3.cdn.digitaloceanspaces.com/sfccy/services/{{ substr($service->img, 0, 1) }}/{{ substr($service->img, 0, 2) }}/{{ substr($service->img, 0, 3) }}/{{ $service->img }}",
    @endif
    "url": "{{ route('web::service', $service->slug) }}",
    ...
    foreach ($categoryIDs as $categoryID) {
        if (!empty($allCategories[$categoryID]) && $allCategories[$categoryID]->parent_id != 0) {
                ...
                <ol role="list" class="flex items-center space-x-4">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <a href="{{ route('web::services') }}" class="ml-4 text-sm font-medium text-gray-400 hover:text-gray-300">{{ __('Services') }}</a>
                        </div>
                    </li>
                    <li>
                        <div class="flex items-center">
                            <svg viewBox="0 0 20 20" fill="currentColor" class="size-5 shrink-0 text-gray-400"><path d="M8.22 5.22a.75.75 0 0 1 1.06 0l4.25 4.25a.75.75 0 0 1 0 1.06l-4.25 4.25a.75.75 0 0 1-1.06-1.06L11.94 10 8.22 6.28a.75.75 0 0 1 0-1.06Z" clip-rule="evenodd" fill-rule="evenodd" /></svg>
                            <span class="ml-4 text-sm font-medium text-gray-500" aria-current="page">{{ Str::limit($service->name, 30) }}</span>
                        </div>
                    </li>
                </ol>
            </nav>
            ...
            <div class="mt-8 space-y-4">
                ...
                @if (!empty($service->languages))
                    <div class="flex items-center text-lg text-gray-300">
                        ...
                        @endphp
                        {{ __('Available in') }} {{ implode(', ', array_slice($languageNames, 0, 3)) }}@if(count($languageNames) > 3) {{ ' +' . (count($languageNames) - 3) . ' ' . __('more') }}@endif
                    </div>
                @endif
                <div class="flex items-center text-lg text-gray-300">
            ...
            <div class="lg:grid lg:grid-cols-12 lg:gap-12">
                <div class="lg:col-span-8">
                    <div class="prose prose-lg max-w-none">
                        <h2 class="text-2xl font-bold text-gray-900 mb-6">{{ __('About this Service') }}</h2>
                        ...
                        @else
                            <p class="text-gray-500 italic">{{ __('No description available for this service.') }}</p>
                        @endif
                    </div>
                    <h3 class="text-lg font-semibold text-gray-900 mt-8 mb-4">{{ __('Service Details') }}</h3>
                    <dl class="space-y-4">
                        <div>
                            <dt class="text-sm font-medium text-gray-500">{{ __('Service Type') }}</dt>
                            <dd class="mt-1 text-sm text-gray-900">{{ __('Professional Service') }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500">{{ __('Availability') }}</dt>
                            <dd class="mt-1 text-sm text-gray-900">
                                @if ($service->active)
                                    <span class="text-green-600 font-semibold">{{ __('Currently Available') }}</span>
                                @else
                                    <span class="text-red-600 font-semibold">{{ __('Currently Unavailable') }}</span>
                                @endif
                            </dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500">{{ __('Service Area') }}</dt>
                            <dd class="mt-1 text-sm text-gray-900">{{ __('Cyprus') }}</dd>
                        </div>
                    </dl>
                    @if (!empty($service->categories))
                        <div class="mt-8">
                    ...
                    <div class="sticky top-8 space-y-6">
                        @if ($service->img)
                            <div class="bg-gray-50 rounded-lg p-6">
                                <img src="https://fvn.ams3.cdn.digitaloceanspaces.com/sfccy/services/{{ substr($service->img, 0, 1) }}/{{ substr($service->img, 0, 2) }}/{{ substr($service->img, 0, 3) }}/{{ $service->img }}" alt="{{ $service->name }}" class="w-full object-contain rounded-lg shadow-lg max-h-48">
                            </div>
                        @endif
                        @if (!empty($service->user->profile->id))
                            <div class="bg-gray-50 rounded-lg p-6">
                                <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ __('Service Provider') }}</h3>
                                <div class="flex items-center space-x-3">
                                    @if ($service->user->img)
                                        <img src="{{ (!empty($service->user->img) ? 'https://fvn.ams3.cdn.digitaloceanspaces.com/sfccy/avatars/' . substr($service->user->img, 0, 1) . '/' . substr($service->user->img, 0, 2) . '/' . substr($service->user->img, 0, 3) . '/th_' . $service->user->img : '/assets/images/no-user-image-icon.png') }}" alt="{{ trim($service->user->profile->first_name . ' '. $service->user->profile->last_name) }}" class="size-10 rounded-full">
                                    @else
                                    ...
                                    @endif
                                    <div>
                                        <p class="font-medium text-gray-900">{{ trim($service->user->profile->first_name . ' '. $service->user->profile->last_name) }}</p>
                                    </div>
                                </div>
                            </div>
                        @endif
                        <div class="bg-gray-50 rounded-lg p-6">
                            <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ __('Share this Service') }}</h3>
...
<div class="bg-gray-100 py-16 sm:py-24 dark:bg-gray-900">
    <div class="mx-auto max-w-7xl px-6 lg:px-8">
        <div class="mx-auto max-w-2xl lg:text-center">
            <h2 class="text-2xl font-bold text-gray-900 mb-6">{{ __('Related Services') }}</h2>
        </div>
                ...
                @foreach ($relatedServices as $relatedService)
                    <li class="relative flex justify-between gap-x-6 px-4 py-5 hover:bg-gray-50 sm:px-6 lg:px-8 dark:hover:bg-white/2.5">
                        <div class="flex min-w-0 gap-x-4">
                            <img src="{{ (!empty($relatedService->img) ? 'https://fvn.ams3.cdn.digitaloceanspaces.com/sfccy/services/' . substr($relatedService->img, 0, 1) . '/' . substr($relatedService->img, 0, 2) . '/' . substr($relatedService->img, 0, 3) . '/th_' . $relatedService->img : '/assets/images/no-image-icon.png') }}" alt="{{ $relatedService->name }}" class="size-12 flex-none rounded-lg bg-gray-50 object-cover dark:bg-gray-800" />
                            <div class="min-w-0 flex-auto">
                                <p class="text-sm/6 font-semibold text-gray-900 dark:text-white">
                                    <a href="{{ route('web::service', $relatedService->slug) }}">
                                ...
                                <p class="mt-1 text-xs/5 text-gray-500 dark:text-orange-400">
                                    @php
                                    $categories = '';
                                    if (!empty($relatedService->categories)) {
                                        $categoryIDs = explode('][', trim($relatedService->categories, '[]'));
                                    ...
                                    @endphp
                                    {{ (!empty($categories) ? $categories . ' • ' : '') }}{{ __('Cyprus') }}
                                </p>
                                @if (!empty($relatedService->description))
                                    <p class="mt-2 text-sm/6 text-gray-600 dark:text-gray-300">{{ Str::limit($relatedService->description, 120) }}</p>
                                @endif
                                @if (!empty($relatedService->languages))
                                    <div class="mt-2 flex items-center gap-x-1">
                                        @php
                                        $allLanguages = config('languages.locales');
                                        $languageIDs = explode('][', trim($relatedService->languages, '[]'));
                                        @endphp
                                        @foreach ($languageIDs as $languageID)
                                            @if (isset($allLanguages[$languageID]))
                        ...
                        <div class="flex shrink-0 items-center gap-x-4">
                            <div class="hidden sm:flex sm:flex-col sm:items-end">
                                @if ($relatedService->active)
                                    <div class="flex items-center gap-x-1.5">
                                        <div class="flex-none rounded-full bg-emerald-500/20 p-1 dark:bg-emerald-500/30">
                                            <div class="size-1.5 rounded-full bg-emerald-500"></div>
                                        </div>
                                        <p class="text-xs/5 text-emerald-600 dark:text-emerald-400 font-medium">{{ __('Available') }}</p>
                                    </div>
                                @else
                                    <div class="flex items-center gap-x-1.5">
                                        ...
                                        <p class="text-xs/5 text-gray-500 dark:text-gray-400">{{ __('Unavailable') }}</p>
                                    </div>
                                @endif
                                <p class="mt-1 text-xs/5 text-gray-500 dark:text-gray-400">{{ __('Professional Service') }}</p>
                            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (shareBtn) {
        shareBtn.addEventListener('click', function() {
            const shareData = {
                title: {!! json_encode($service->name) !!},
                text: {!! json_encode(Str::limit(strip_tags($service->description ?? ''), 100)) !!},
...
<div class="bg-white rounded-lg p-6 max-w-sm mx-4 w-full">
    <div class="flex justify-between items-center mb-4">
        <h3 class="text-lg font-semibold text-gray-900">{{ __('Share this Service') }}</h3>
...
function setupSocialShareLinks() {
    const serviceTitle = {!! json_encode($service->name) !!};
    const serviceDescription = {!! json_encode(Str::limit(strip_tags($service->description ?? ''), 100)) !!};
    const serviceUrl = window.location.href;

    // Facebook share
    const facebookBtn = document.getElementById('facebook-share');
    if (facebookBtn) {
        facebookBtn.href = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(serviceUrl)}`;
    }

    // Twitter share
    const twitterBtn = document.getElementById('twitter-share');
    if (twitterBtn) {
        const tweetText = `${serviceTitle} - Professional Service in Cyprus`;
        twitterBtn.href = `https://twitter.com/intent/tweet?text=${encodeURIComponent(tweetText)}&url=${encodeURIComponent(serviceUrl)}`;
    }

    // WhatsApp share
    const whatsappBtn = document.getElementById('whatsapp-share');
    if (whatsappBtn) {
        const whatsappText = `Check out this service: ${serviceTitle} ${serviceUrl}`;
    ...
    if (linkedinBtn) {
        linkedinBtn.href = `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(serviceUrl)}`;
    }
