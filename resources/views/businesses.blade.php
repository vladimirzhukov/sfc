...
$breadcrumbItems = collect([['name' => __('Home'), 'url' => route('web::index')], ['name' => __('Businesses'), 'url' => route('web::businesses')]]);
if (Route::currentRouteName() == 'web::events::filter') {
    $filterPages = ['businesses/recent' => __('Recently Added'), 'businesses/cities' => __('Choose City')];
    foreach ($filterPages as $page => $pageName) {
        if (str_contains(request()->getRequestUri(), $page)) {
            $breadcrumbItems->push([
                'name' => $pageName,
                'url' => route('web::events::filter', ['link' => str_replace('businesses/', '', $page)])
            ]);
            break;
        }
    }
    if (!$breadcrumbItems->count() > 2 && !empty($currentCategory->id)) {
        $breadcrumbItems->push([
            ...
            'url' => route('web::businesses::filter', ['link' => $currentCategory->slug])
        ]);
    }
    if (!$breadcrumbItems->count() > 2 && !empty($currentCity->id)) {
        $breadcrumbItems->push([
            'name' => $currentCity->name,
            'url' => route('web::businesses::filter', ['link' => $currentCity->slug])
        ]);
    }
} elseif (Route::currentRouteName() == 'web::businesses::filter::category' && !empty($currentCategory->parent_id)) {
    if (!empty($allCategories[$currentCategory->parent_id]->id)) {
        $breadcrumbItems->push([
            'name' => $allCategories[$currentCategory->parent_id]->name,
            'url' => route('web::businesses::filter', ['link' => $allCategories[$currentCategory->parent_id]->slug])
        ]);
    }
    $breadcrumbItems->push([
        'name' => $currentCategory->name,
        'url' => route('web::businesses::filter::category', [
...
<div class="relative isolate overflow-hidden pt-14">
    <div class="relative isolate overflow-hidden bg-gray-900 px-6 py-20 sm:rounded-3xl sm:px-10 sm:py-24 lg:py-24 xl:px-24 dark:bg-gray-800/50 dark:after:pointer-events-none dark:after:absolute dark:after:inset-0 dark:after:inset-ring dark:after:inset-ring-white/10 dark:after:sm:rounded-3xl">
        <div class="mx-auto max-w-4xl text-center">
            <h1 class="text-4xl font-semibold tracking-tight text-balance text-white sm:text-5xl">{{ (!empty($meta->h1) ? $meta->h1 : __('Business Directory in Cyprus')) }}</h1>
            <p class="mt-6 text-lg/8 text-gray-300">{{ (!empty($meta->subtext) ? $meta->subtext : __('web.events_1')) }}</p>
            <div class="mt-10 flex flex-wrap justify-center gap-4">
                <a href="{{ route('signup') }}" class="rounded-md bg-orange-500 px-4 py-2 text-sm font-semibold text-white shadow-xs hover:bg-orange-400 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-orange-500">{{ __('Submit Business') }}</a>
                <a href="{{ route('web::businesses::filter', ['link' => 'recent']) }}" class="rounded-md bg-white/10 px-4 py-2 text-sm font-semibold text-white shadow-xs hover:bg-white/20 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white">{{ __('Recently Added') }}</a>
            </div>
            <div class="mt-10 pt-8 border-t border-white/10">
                <div class="grid grid-cols-2 gap-8 sm:grid-cols-4">
                    <div>
                        <div class="text-2xl font-bold text-white">150+</div>
                        <div class="text-sm text-gray-400">{{ __('Businesses') }}</div>
                    </div>
                    <div>
    ...
    <div class="mx-auto max-w-7xl px-6 py-16 lg:px-8 lg:py-24">
        <div class="lg:grid lg:grid-cols-12 lg:gap-8">
                                ...
                                <div class="space-y-2">
                                    ...
                                    @if ($categories->isNotEmpty())
                                        @foreach ($categories as $category)
                                            ...
                                            @endphp
                                            @if (!empty($currentCategory->id) && empty($currentCategory->parent_id))
                                                <a href="{{ route('web::businesses::filter::category', ['link' => $currentCategory->slug, 'category' => $category->slug]) }}" class="block text-sm text-gray-600 dark:text-gray-400 hover:text-orange-500 dark:hover:text-orange-400 transition-colors">{{ $name }}</a>
                                            @elseif (!empty($currentCategory->id) && !empty($currentCategory->parent_id))
                                                <a href="{{ route('web::businesses::filter::category', ['link' => $allCategories[$currentCategory->parent_id]->slug, 'category' => $category->slug]) }}" class="block text-sm text-gray-600 dark:text-gray-400 hover:text-orange-500 dark:hover:text-orange-400 transition-colors">{{ $name }}</a>
                                            @else
                                                <a href="{{ route('web::businesses::filter', ['link' => $category->slug]) }}" class="block text-sm text-gray-600 dark:text-gray-400 hover:text-orange-500 dark:hover:text-orange-400 transition-colors">{{ $name }}</a>
                                            @endif
                                        @endforeach
                                    @endif
                                </div>
                                ...
                                <div class="space-y-2">
                                    @if ($cities->isNotEmpty())
                                        @foreach ($cities as $city)
                                            <a href="{{ route('web::businesses::filter', ['link' => $city->slug]) }}" class="block text-sm text-gray-600 dark:text-gray-400 hover:text-orange-500 dark:hover:text-orange-400 transition-colors">{{ $city->name }}</a>
                                        @endforeach
                                    @endif
                                    <a href="{{ route('web::businesses::filter', ['link' => 'cities']) }}" class="block text-sm text-gray-600 dark:text-gray-400 hover:text-orange-500 dark:hover:text-orange-400 transition-colors">All Cities</a>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-6">
                        ...
                        <div class="space-y-2">
                            @if ($cities->isNotEmpty())
                                ...
                                @endforeach
                            @endif
                            <a href="{{ route('web::businesses::filter', ['link' => 'cities']) }}" class="block text-sm text-gray-600 dark:text-gray-400 hover:text-orange-500 dark:hover:text-orange-400 transition-colors">{{ __('All Cities') }}</a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="lg:col-span-9">
                            ...
                            <ol role="list" class="flex items-center space-x-4 mb-4">
                                ...
                                <li>
                                    <div class="flex items-center">
                                        ...
                                        <a href="{{ route('web::businesses') }}" class="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">{{ __('Businesses') }}</a>
                                    </div>
                                </li>
                                @if (Route::currentRouteName() == 'web::businesses::filter')
                                    @php
                                    $pages = array('businesses/recent' => __('Recently Added'), 'businesses/cities' => __('Choose City'));
                                    $isPage = null;
                                    ...
                                    @if (!empty($isPage))
                                        <li>
                                            <div class="flex items-center">
                                                <svg viewBox="0 0 20 20" fill="currentColor" data-slot="icon" aria-hidden="true" class="size-5 shrink-0 text-gray-400 dark:text-gray-500"><path d="M8.22 5.22a.75.75 0 0 1 1.06 0l4.25 4.25a.75.75 0 0 1 0 1.06l-4.25 4.25a.75.75 0 0 1-1.06-1.06L11.94 10 8.22 6.28a.75.75 0 0 1 0-1.06Z" clip-rule="evenodd" fill-rule="evenodd" /></svg>
                                                <a href="{{ route('web::businesses::filter', ['link' => str_replace('businesses/', '', $isPage)]) }}" aria-current="page" class="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">{{ $pages[$isPage] }}</a>
                                            </div>
                                        </li>
                                    @elseif (!empty($currentCategory->id))
                                        <li>
                                            <div class="flex items-center">
                                                <svg viewBox="0 0 20 20" fill="currentColor" data-slot="icon" aria-hidden="true" class="size-5 shrink-0 text-gray-400 dark:text-gray-500"><path d="M8.22 5.22a.75.75 0 0 1 1.06 0l4.25 4.25a.75.75 0 0 1 0 1.06l-4.25 4.25a.75.75 0 0 1-1.06-1.06L11.94 10 8.22 6.28a.75.75 0 0 1 0-1.06Z" clip-rule="evenodd" fill-rule="evenodd" /></svg>
                                                <a href="{{ route('web::businesses::filter', ['link' => $currentCategory->slug]) }}" aria-current="page" class="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">{{ $currentCategory->name }}</a>
                                            </div>
                                        </li>
                                    @elseif (!empty($currentCity->id))
                                        <li>
                                            <div class="flex items-center">
                                                <svg viewBox="0 0 20 20" fill="currentColor" data-slot="icon" aria-hidden="true" class="size-5 shrink-0 text-gray-400 dark:text-gray-500"><path d="M8.22 5.22a.75.75 0 0 1 1.06 0l4.25 4.25a.75.75 0 0 1 0 1.06l-4.25 4.25a.75.75 0 0 1-1.06-1.06L11.94 10 8.22 6.28a.75.75 0 0 1 0-1.06Z" clip-rule="evenodd" fill-rule="evenodd" /></svg>
                                                <a href="{{ route('web::businesses::filter', ['link' => $currentCity->slug]) }}" aria-current="page" class="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">{{ $currentCity->name }}</a>
                                            </div>
                                        </li>
                                    @endif
                                @elseif (Route::currentRouteName() == 'web::businesses::filter::category')
                                    <li>
                                        <div class="flex items-center">
                                            <svg viewBox="0 0 20 20" fill="currentColor" data-slot="icon" aria-hidden="true" class="size-5 shrink-0 text-gray-400 dark:text-gray-500"><path d="M8.22 5.22a.75.75 0 0 1 1.06 0l4.25 4.25a.75.75 0 0 1 0 1.06l-4.25 4.25a.75.75 0 0 1-1.06-1.06L11.94 10 8.22 6.28a.75.75 0 0 1 0-1.06Z" clip-rule="evenodd" fill-rule="evenodd" /></svg>
                                            <a href="{{ route('web::businesses::filter', ['link' => $allCategories[$currentCategory->parent_id]->slug]) }}" aria-current="page" class="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">{{ $allCategories[$currentCategory->parent_id]->name }}</a>
                                        </div>
                                    </li>
                                    <li>
                                        <div class="flex items-center">
                                            <svg viewBox="0 0 20 20" fill="currentColor" data-slot="icon" aria-hidden="true" class="size-5 shrink-0 text-gray-400 dark:text-gray-500"><path d="M8.22 5.22a.75.75 0 0 1 1.06 0l4.25 4.25a.75.75 0 0 1 0 1.06l-4.25 4.25a.75.75 0 0 1-1.06-1.06L11.94 10 8.22 6.28a.75.75 0 0 1 0-1.06Z" clip-rule="evenodd" fill-rule="evenodd" /></svg>
                                            <a href="{{ route('web::businesses::filter::category', ['link' => $allCategories[$currentCategory->parent_id]->slug, 'category' => $currentCategory->slug]) }}" aria-current="page" class="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">{{ $currentCategory->name }}</a>
                                        </div>
                                    </li>
                                @endif
                            </ol>
                                    ...
                                    @foreach ($columns as $column)
                                        <div class="space-y-1">
                                            @foreach ($column as $city)
                                                <a href="{{ route('web::businesses::filter', ['link' => $city->slug]) }}" class="block text-sm text-blue-600 hover:text-blue-800 hover:underline transition-colors">
                                                    {{ $city->name }}
                        ...
                        <ul role="list" class="divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                @foreach ($businesses as $business)
                                    <li class="relative flex justify-between gap-x-6 px-4 py-5 hover:bg-gray-50 sm:px-6 lg:px-8 dark:hover:bg-white/2.5">
                                        <div class="flex min-w-0 gap-x-4">
                                            <img src="{{ (!empty($business->img) ? 'https://fvn.ams3.cdn.digitaloceanspaces.com/sfccy/business/' . substr($business->img, 0, 1) . '/' . substr($business->img, 0, 2) . '/' . substr($business->img, 0, 3) . '/th_' . $business->img : '/assets/images/no-image-icon.png') }}" alt="{{ $business->name }}" class="size-12 mt-2 flex-none rounded-lg bg-gray-50 object-cover dark:bg-gray-800" />
                                            <div class="min-w-0 flex-auto">
                                                <p class="text-sm/6 font-semibold text-gray-900 dark:text-white">
                                                    <a href="{{ route('web::business', $business->slug) }}">
                                                ...
                                                <p class="mt-1 text-xs/5 text-gray-500 dark:text-orange-400">
                                                    ...
                                                    @endphp
                                                    {{ (!empty($categories) ? $categories . ' • ' : '') }}{{ (!empty($business->is_online) ? __('Online') : (!empty($business->city->name) ? $business->city->name . (!empty($business->city->region->name) ? ', ' . $business->city->region->name : '') : __('Cyprus'))) }}
                                                </p>
                                                ...
                                                <div class="mt-2 flex items-center gap-x-4">
                                                    @if (!empty($business->location))
                                                        <div class="flex items-center gap-x-1.5">
                                                            ...
                                                            <p class="text-xs/5 text-gray-500 dark:text-gray-400">{{ Str::limit($business->location, 40) }}</p>
                                                        </div>
                                                    @endif
                                                    @if (!empty($business->link))
                                                        <div class="flex items-center gap-x-1.5">
                                                            <svg class="size-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M13.19 8.688a4.5 4.5 0 0 1 1.242 7.244l-4.5 4.5a4.5 4.5 0 0 1-6.364-6.364l1.757-1.757m13.35-.622 1.757-1.757a4.5 4.5 0 0 0-6.364-6.364l-4.5 4.5a4.5 4.5 0 0 0 1.242 7.244" /></svg>
                                                            <p class="text-xs/5 text-gray-500 dark:text-gray-400">{{ __('Website') }}</p>
                                                        </div>
                                                    @endif
                                                </div>
                                        ...
                                        <div class="flex shrink-0 items-center gap-x-4">
                                            <div class="hidden sm:flex sm:flex-col sm:items-end">
                                                @if (!empty($business->created_at))
                                                    <p class="mt-1 text-xs/5 text-gray-500 dark:text-gray-400">
                                                        {{ __('Added') }} {{ \Carbon\Carbon::parse($business->created_at)->format('M j, Y') }}
                                                    </p>
                                                @endif
                                            </div>
                            ...
                            @else
                                <li class="px-4 py-12 text-center">
                                    ...
                                    <h3 class="mt-2 text-sm font-semibold text-gray-900 dark:text-white">{{ __('No businesses found') }}</h3>
                                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('Check back later for businesses or try different filters.') }}</p>
                                    <div class="mt-6">
                ...
                @endif
                @if (!empty($businesses) && $businesses->isNotEmpty() && method_exists($businesses, 'links'))
                    <div class="mt-8">
                        {{ $businesses->links() }}
                    </div>
                @endif
